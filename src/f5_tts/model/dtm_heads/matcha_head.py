...
class MLPBlock(nn.Module):
    ...
    def __init__(self, dim, time_emb_dim):
        super().__init__()
        self.norm = AdaLayerNorm(dim, time_emb_dim)
...
class Matcha(nn.Module):
    def __init__(
        ...
        for i in range(len(channels)):
            ...
            # MLP Blocks
            mlp_blocks = nn.ModuleList([self.get_block(output_channel, time_embed_dim) for _ in range(n_blocks)])
        ...
        for i in range(num_mid_blocks):
            ...
            mlp_blocks = nn.ModuleList([self.get_block(mid_channel, time_embed_dim) for _ in range(n_blocks)])
    ...
    @staticmethod
    def get_block(dim, time_emb_dim):
        """Returns a simple MLP block (Linear -> SnakeBeta -> Linear)"""
        return MLPBlock(dim, time_emb_dim)
...
class MatchaDTMHead(nn.Module):
    ...
    def __init__(
        ...
        self.unet = Matcha(
            ...
            out_channels=mel_dim,     # 最终输出 Mel 维度
